...
class TD3():
    def __init__(self,n_games = 1000,render=False):
        if render:
            self.env = gym.make('BipedalWalker-v3', render_mode='human')
        else:
            self.env = gym.make('BipedalWalker-v3')
        self.agent = Agent(alpha=0.001, beta=0.001,
        input_dims=self.env.observation_space.shape, tau=0.005,
        env=self.env, batch_size=100, layer1_size=400, layer2_size=300,
        ...
        wandb.init(
            ...
            config={
            "algorithm": "TD3",
            "batch size":self.agent.batch_size,
            "alpha": self.agent.alpha,
            "beta": self.agent.gamma,
            "layer 1 size":self.agent.layer1_size,
            "layer 2 size":self.agent.layer2_size,
            "episodes": n_games,
            })
