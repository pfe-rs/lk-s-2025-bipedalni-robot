...
class ReplayBuffer(): #cuva prethodno nauceno
    ...
    def state_in_last(self):
        index = self.mem_counter % self.mem_size
        if index==0:
            index=self.mem_size-1
        else:
            index -=1
        return self.state_memory[index]
...
class Agent():
    ...
    def add_reward(self,state):
        last_state=self.memory.state_in_last()
        reward=(state[2]+last_state[2])/2-0.15
        if state[14]<state[15] or state[15]<state[16] or state[16]<state[17]:
            reward+=(state[3]+last_state[3])/2-0.1
        return reward
...
env = RecordVideo(env,
                video_folder="td3_hardcore_video",
                name_prefix="eval",
...
for i in range(n_games):
    ...
    while not done:
        ...
        score_of_the_game += reward
        reward+=agent.add_reward(observation_)
        our_score+=reward
        agent.remember(observation, action, reward, observation_, done)
        agent.learn()
        observation = observation_
    wandb.log({"episode": i, "score": score_of_the_game})
    score_history.append(score_of_the_game)
    ...
    print('episode ', i, 'score %.1f' % score_of_the_game, 'new score %.1f'%our_score,
